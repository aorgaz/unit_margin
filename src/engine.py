...
import datetime
import pytz
from src import config, processing, utils
from src.readers import i90_reader, omie_reader, esios_reader

def get_price_data(mkt, date_obj):
    """
    Retrieves price data based on market configuration.
    """
    ...
    if source == 'indicator':
        ...
        if isinstance(price_id, int) or isinstance(price_id, str):
            ...
            price_df = esios_reader.read_esios_indicator(fpath)
        ...
        # Need to read marginalpdbc or similar
        data_id_price = mkt.get('price_id') # E.g. "marginalpdbc"
        
        # NOTE: Config structure: data_id="pdbc", price_id="marginalpdbc"
        # We assume price files are in their own folders or same base folder?
        # Folder structure seems to be: DATA/OMIE/zip/<data_id_price>/...
        # Let's verify if price_id matches folder name.
        # Check if we have specific folder for price_id
        ...
        if data_id_price:
             ...
             price_df = omie_reader.read_omie_file(zip_path, inner_prefix)
        ...
        if price_id:
            ...
            for sheet in sheet_ids:
                d = i90_reader.read_i90_zip(zip_path, sheet)
                if not d.empty:
                    dfs.append(d)
                    ...
                    if not price_df.empty:
                        ...
                        if cols:
                            ...
                            if val_cols:
                                ...
                                unit_candidates = ['UNIDAD DE PROGRAMACIÓN', 'UNIDAD DE PROGRAMACION', 'PARTICIPANTE DEL MERCADO']
                                unit_col = next((c for c in price_df.columns if c.upper() in unit_candidates), None)
                                if unit_col:
...
def process_market(mkt, date_obj, target_units):
    logger = logging.getLogger()
    ymd = date_obj.strftime("%Y%m%d")
    ...
    # region --- 1. READ QUANTITY ---
    df_qty = pd.DataFrame()
    
    try:
        # region --- PROCESSING I90 ---
        if data_source == 'i90':
            zip_name = f"I90DIA_{ymd}.zip"
            zip_path = os.path.join(config.PATH_ESIOS_I90, f"i90_{year}", zip_name)
            ...
            if dfs:
                ...
                if not df_qty.empty:
                    ...
                    # Identify Unit Col
                    unit_candidates = ['UNIDAD DE PROGRAMACIÓN', 'UNIDAD DE PROGRAMACION', 'PARTICIPANTE DEL MERCADO']
                    unit_col = next((c for c in df_qty.columns if c.upper() in unit_candidates), None)
        ...
        # region --- PROCESSING OMIE ---
             ...
             df_qty = omie_reader.read_omie_file(zip_path, inner_prefix)
                         ...
                         if 'SESSION' in df_qty.columns:
                             ...
                             if isinstance(price_map, dict):
                                 ...
                                 for sess in unique_sessions:
                                     ...
                                     p_df = esios_reader.read_esios_indicator(fpath)
    ...
    if market_name == 'MIC' or market_name == 'PIBC': # PIBC handled internally above
        ...
        df_price = get_price_data(mkt, date_obj)
    # endregion
    
    # region --- 3. MERGE & CALCULATE ---
    ...
    if market_name == 'MIC' or market_name == 'PIBC':
        ...
        # Normalize Price to match Qty resolution if needed
        # df_qty has 'Datetime_Madrid'. df_price should have datetime too.
        
        # Standardize Price DF
        # Indicators from ESIOS usually have 'datetime_utc' or similar. 
        # Need to parse and convert to Madrid to match.
        pass # Placeholder for merge details (assumed df_price prepared)
        
        # Merge
        # We need common key 'Datetime_Madrid'
        # Check if Price has 'Datetime_Madrid', if not try to infer
        price_cols = df_price.columns
        dt_col_price = next((c for c in price_cols if 'DATETIME' in c.upper() or 'FECHA' in c.upper()), None)
        # If Indicator: usually 'datetime_utc'
        # Assuming esios_reader returns 'datetime_utc' or similar.
        # We did not implement esios_reader normalization yet.
        # Minimal fix:
        if dt_col_price and 'Datetime_Madrid' not in df_price.columns:
             # Try convert?
             # For now, let's assume get_price_data handled standardisation or we skip.
             pass
    ...
    for s_date, e_date in date_ranges:
        ...
        while current <= e_date:
            ...
            for mkt in config.MARKET_CONFIG:
                ...
                df_mkt = process_market(mkt, current, target_units)
                if not df_mkt.empty:
